...
namespace uServiceDemo.Infrastructure.Repositories.Mappings
{
    static class WeatherForecastEntityMap
    {
        public static void Map(EntityTypeBuilder<WeatherForecastEntity> entityTypeBuilder)
        {
            ...
            entityTypeBuilder.Property(x => x.Created).ValueGeneratedOnAdd().HasDefaultValueSql("GETUTCDATE()");
            entityTypeBuilder.Property(x => x.LastUpdated).ValueGeneratedOnAddOrUpdate().HasDefaultValueSql("GETUTCDATE()");
